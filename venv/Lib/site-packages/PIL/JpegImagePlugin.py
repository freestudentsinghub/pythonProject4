...
# 1997-08-27 fl   Save mode 1 media as BW (0.3)
...
def SOF(self: JpegImageFile, marker: int) -> None:
    ...
    if self.layers == 1:
        ...
        msg = f"cannot handle {self.layers}-layer media"
        raise SyntaxError(msg)
...
##
# Image plugin for JPEG and JFIF media.
...
def _getmp(self: JpegImageFile) -> dict[int, Any] | None:
    ...
    # it's an error not to have a number of media
    try:
        quant = mp[0xB001]
    except KeyError as e:
        msg = "malformed MP Index (no number of media)"
        raise SyntaxError(msg) from e
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im: Image.Image) -> int:
    # There's no subsampling when media have only 1 layer
    # (grayscale media) or when they are CMYK (4 layers),
        ...
        if mpheader is not None and mpheader[45057] > 1:
            for segment, content in im.applist:
                if segment == "APP1" and b' hdrgm:Version="' in content:
                    # Ultra HDR media are not yet supported
                    return im
